...
const AboutMe = () => {
    const skillList = skills.map((skill, idx) => <img src={skill.icon} alt={`${skill.name}`} title={`${skill.name}`}className="rounded-lg w-28 m-2" />);
        ...
        <section id="about" className="py-32 px-20 h-screen bg-neutral-100 text-blue-950 flex justify-between">
            ...
            <div className="basis-1/3 flex flex-col">
                ...
                <div className="flex justify-center">
                    <div className="flex flex-wrap mx-4">{skillList}</div>
                </div>
            </div>
        </section>
    );
